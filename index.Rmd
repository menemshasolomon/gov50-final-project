...
voteridplot
```
Similar to automatic registration, there appears to be more support for voter id laws than opposition. That said, there are fewer individuals — under 1500 — who support these laws strongly and nearly 500 respondents strongly oppose this policy.
...
```
The bootstrap replications display the number of samples which result in a particular difference in means. For the graph of views on automatic registration, the average treatment effect follows a normal distribution with the center located around 0.57, meaning on average respondents who support voter ID laws were less in favor of automatic registration than those who did not by about 0.57 points. Furthermore, the 95% confidence interval calculated above has a lower bound of 0.486 and an upper bound of 0.638, thus it does not include zero. Thus, at least at the 95% confidence interval, it is possible to discount the null hypothesis — support for voter ID laws bears no effect on a respondent's views on automatic registration.

Multiple Regression
---

To further explore this relationship, I conducted a regression which not only analyzed the impact of my primary independent variable but also my other independent variables. This regression analyzes the effect of respondents' perspective on voter ID laws, race, party, gender, and age on their views about automatic registration.
...
fit_ate_voterid <- 
  lm(automaticregistration ~ voteridbinary + race + party + gender + age, data = data2)
...
```
This regression indicates that relative to those who oppose voter ID laws, an individual who supports voter ID laws scores 0.148 points higher on their feeling towards automatic registration: meaning they oppose the policy by 0.148 points more than their peers who do not support voter ID laws. The p-value of 0.005 indicates that this result is highly significant. Furthermore, this information indicates that we can not only reject the null hypothesis of no effect at the 0.05 level but at the 0.01 level as well, as 0.005 is less than 0.01.

The regression above also analyzes the effect of other independent, and possibly explanatory, variables. To evaluate if the effect of these other variables is statistically significant, it is important to first look at their p-values. Only age and party have a p-value under 0.05, the value which indicates statistical significance. This data indicates that neither race nor gender have a significant impact on a respondent's view toward automatic registration.
...
```
This graph indicates a clear relationship between party membership and support of automatic registration, wherein around 1750 members of the democratic party report supporting automatic registration to some degree. Furthermore, while around 500 members of the republican party oppose automatic registration, under 250 democrats feel similarly. 

Conclusion 
---

The initial hypothesis stated that perspective on automatic registration will correlate with perspective on voter ID laws wherein individuals who oppose voter ID laws, a policy with a longstanding history of voter discrimination, will support automatic registration, a policy which would increase access to voting. Both the bootstrapped difference in means and the regression model indicate that those who support voter ID laws tend to oppose automatic registration. That said, while the null hypothesis of no effect was rejected, the effect was found to be quite small. The next part of the hypothesis stated that I expected this relationship to be in large part explained first by party membership, as democrats typically support progressive voting policies, and second by racial identity, as marginalized racial groups are historically the most affected by voter disenfranchisement. Surprisingly, the regrssion model found that the relationship between race and perspective on automatic registration was not statistically significant. Two other independent variables — age and party membership — were, however, statistically significant, and for both, the null hypothesis of no effect could be rejected. Indeed, an increase in age appeared to correlate with a small increase in opposition toward automatic registration. Furthermore, the coefficient for party membership was the largest, indicating that democrats tended to favor automatic registration by 1.419 points more than republican respondents. 

These results beget the conclusion that passing automatic registration policy is not seen as the same as abolishing voter ID laws in the United States. Indeed, while those who support voter id laws correlate somewhat with those who support automatic registration, the explanatory variable with the largest impact on perspective is party membership. Despite this, there is not enough data in this study to determine any sort of causal relationship, merely assert that there is definitely correlation. To conduct a more comprehensive analysis of perspective on the various influences which determine an individual's perspective on voter registration, it is important to look at many policies across time. 
