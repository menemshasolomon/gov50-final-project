...
```{r}
automatic_registration_plot <- data |>
  filter(automaticregistration!=99)  |>
  ggplot(mapping = aes(x = automaticregistration)) +
  geom_bar() 
automatic_registration_plot
```

```{r}
automatic_registration_party <- data |>
  filter(party!=4, party!=99, automaticregistration!=99) |>
  group_by(party) |>
  ggplot(mapping = aes(x = automaticregistration)) +
  geom_bar() 
automatic_registration_party 
